...
- **Estado:** guion en `borrador` con el Bloque 7 pendiente. En el índice (`lib/charlas.ts`) va como `publicada` para que enlace desde la home.
- **Formato:** charla de **teoría / modelos mentales**. **NO hay demo en directo** — decisión consciente: nada frágil que dependa de la red. ~46 min. Audiencia ~168 personas, 98% no técnica.
...
7. Bloque 7 — Visual guiado (**placeholder, contenido pendiente**)
...
- Bloque 4: «El mismo trabajo que hace el ejecutor cerrado. Pero el motor lo eliges tú, y tus datos se quedan en casa.»
...
- `visual-section-20.tsx` — Bloque 7 (**placeholder**)
...
## Decisión abierta (bloquea el Bloque 7)

`visual-section-20.tsx` está montado pero **sin contenido real**. Juan quiere combinar un recorrido por el sitio de OpenCode y un walkthrough de la herramienta ya montada; pasará el material antes de impartir. Al rellenarlo:

- Sustituir el bloque marcado como «Contenido pendiente» por los pasos reales.
- **No mover el reseñable objetivo:** el instante en que se elige el proveedor / modelo. Ese es el momento que la audiencia tiene que ver.

## Límites de contenido

- **Sin cifras exactas** de OpenCode (nº de proveedores, ranking de popularidad). El guion avisa de no colgarse de un número: la web dice «decenas de proveedores» y «de las más usadas», y así debe quedarse salvo verificación previa.
